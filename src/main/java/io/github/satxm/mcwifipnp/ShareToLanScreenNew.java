...
public class ShareToLanScreenNew extends Screen {
	...
	protected void init() {
		...
		this.EditPlayers = new EditBox(this.font, this.width / 2 - 48, 70, 96, 20,
				new TextComponent(Integer.toString(cfg.maxPlayers)));
		...
		this.EditMotd = new EditBox(this.font, this.width / 2 + 58, 70, 96, 20, new TextComponent(cfg.motd));
		...
		this.addRenderableWidget(CycleButton.onOffBuilder(cfg.AllPlayersCheats).create(this.width / 2 - 155, 124, 150, 20,
				new TranslatableComponent("mcwifipnp.gui.AllPlayersCheats"), (button, AllPlayersCheats) -> {
		...
		this.addRenderableWidget(CycleButton.onOffBuilder(cfg.Whitelist).create(this.width / 2 + 5, 124, 150, 20,
				new TranslatableComponent("mcwifipnp.gui.Whitelist"), (button, Whitelist) -> {
		...
		this.addRenderableWidget(CycleButton.onOffBuilder(cfg.PvP).create(this.width / 2 + 5, 148, 150, 20,
				new TranslatableComponent("mcwifipnp.gui.PvP"), (button, PvP) -> {
					cfg.PvP = PvP;
				}));
	...
	public void render(PoseStack poseStack, int i, int j, float f) {
		...
		drawString(poseStack, this.font, new TranslatableComponent("mcwifipnp.gui.port"), this.width / 2 - 149, 58,
				16777215);
		drawString(poseStack, this.font, new TranslatableComponent("mcwifipnp.gui.players"), this.width / 2 - 43, 58,
				16777215);
		drawString(poseStack, this.font, new TranslatableComponent("mcwifipnp.gui.motd"), this.width / 2 + 63, 58,
				16777215);
